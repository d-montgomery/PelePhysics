      ...
      amrex::ParallelFor(Np, [=] AMREX_GPU_DEVICE(int pid) noexcept {
        ...
        if (p.id() > 0) {
          ...
          for (int cur_iter = 0; cur_iter < num_iter && p.id() > 0;
            ...
            if (fdat->mom_trans) {
              amrex::Print(myrank) << myrank << ":    Gpu::Atomic::Add(&momSrcarr)\n";
              for (int dir = 0; dir < AMREX_SPACEDIM; ++dir) {
                Gpu::Atomic::Add(
            ...
            if (fdat->mass_trans) {
              amrex::Print(myrank) << myrank << ":    Gpu::Atomic::Add(&rhoSrcarr)\n";
              amrex::Print(myrank) << myrank << ":       N_pc = " << fdat->N_pc << "\n";
              Gpu::Atomic::Add(
                &rhoSrcarr(cur_indx), cur_coef * gpv.fluid_mass_src);
              for (int pc = 0; pc < fdat->N_pc; ++pc) {
                const int pcspec = fdat->pc_indx[pc];
                amrex::Print(myrank) << myrank << ":       pc = " << pc <<", pcspec = " << pcspec<<"\n";
                amrex::Print(myrank) << myrank << ":       gpv.fluid_Y_dot["<<pcspec<<"] = " << gpv.fluid_Y_dot[pcspec] << "\n";
                Gpu::Atomic::Add(
                  &rhoYSrcarr(cur_indx, pc),
                  cur_coef * gpv.fluid_Y_dot[pcspec]);
                amrex::Print(myrank) << myrank << ":       added gpv.fluid_Y_dot["<<pcspec<<"] \n";
              }
            }
            amrex::Print(myrank) << myrank << ":    Gpu::Atomic::Add(&engSrcarr)\n";
            Gpu::Atomic::Add(
